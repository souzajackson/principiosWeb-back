...
export const createAdoption = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (error instanceof NotFoundError || error instanceof BadRequestError) return next(error);
...
export const getAdoptionById = async (req: Request, res: Response, next: NextFunction) => {
  ...
  export const getMyAdoptions = async (
    ...
    try {
      ...
      const adoptions = await service.getMyAdoptions(userId);
      ...
      return res.status(500).json({ message: "Error fetching my adoptions" });
    }
  };

  // GET /adoptions/shelter
  export const getMyShelterAdoptions = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      ...
      const adoptions = await service.getMyShelterAdoptions(userId);
      ...
      console.error("Error fetching shelter adoptions:", error);
      return res.status(500).json({ message: "Error fetching shelter adoptions" });
    }
  };
...
export const approveAdoption = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = (req as any).user.id;
...
export const rejectAdoption = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = (req as any).user.id;
    const adoptionId = Number(req.params.id);
