...
export class AdoptionService {
  ...
  async getMyAdoptions(userId: number) {
    const user = await this.userService.getUserById(userId);
    if (!user) throw new NotFoundError("Usuário não encontrado");

    return this.repo.getAdoptionsByUserId(userId);
  }

  async getMyShelterAdoptions(userId: number) {
    const user = await this.userService.getUserById(userId);
    if (!user) throw new NotFoundError("Usuário não encontrado");

    if (user.role !== "SHELTER" && user.role !== "SUPER") {
      throw new Forbidden("Apenas abrigos podem visualizar estas solicitações");
    }

    const shelter = await this.shelterService.getShelterByUserId(userId);
    if (!shelter) {
      throw new NotFoundError("Abrigo não encontrado para este usuário");
    }

    return this.repo.getAdoptionsByShelterId(shelter.id);
  }
  ...
  private async verifyAdoptionAndUser(adoptionId: number, userId: number) {
    ...
    if (adoption.status !== "PENDING")
      throw new BadRequestError("Não é possível aceitar ou rejeitar adoções que não estão pendentes.");
    ...
    if (userId !== shelter.userId && user.role !== "SUPER")
      throw new Forbidden("Abrigo não é o responsável por essa adoção");
  }
}